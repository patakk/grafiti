...
var resx = 1366;
var resy = Math.floor(resx/1.414);
let ratio = window.innerWidth/window.innerHeight;
resy = resx/ratio;
if(ratio > 1){
  resy = 1366;
  resx = Math.floor(resy*ratio);
}
...
function initialParticleData(num_parts) {
  ...
  for (var i = 0; i < num_parts; ++i) {
    ...
    // pos
    let x = map(fxrand(), 0, 1, .3/ratio, -.3/ratio+1)*resx;
    let y = map(fxrand(), 0, 1, .3, -.3+1)*resy;
    data.push(x);
    data.push(y);
...
function render(gl, state, timestamp_millis) {
  ...
  gl.uniform3f(
    ...
    0,0,0);
  gl.uniform1f(
    gl.getUniformLocation(state.particle_render_program, "u_Scale"),
    .75);
...
function resetState(){
  ...
  document.onmousemove = function(e) {
    document.getElementById("info").innerHTML = "naasfas"
    ...
    if(mousepressed && x > 0 && y > 0 && pmouseX > 0 && pmouseY > 0){
      movementspeed = map(constrain(d, 0, 100), 0, 100, 1, 5);
  ...
  canvas.ontouchstart = function(e) {
    ...
    for(var i = 0; i < evt.touches.length; i++){
      ...
      //   pmouseY = thestate.origins[0][1];
      // }
      movementangle = random(-.2, .2);
      movementspeed = 1;
      thestate.origins[i] = [x, y];
    }
    document.getElementById("info").innerHTML = thestate.origins[0][0] + " " + thestate.origins[0][1]
  };
